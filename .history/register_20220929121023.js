...
const { buildXMLFromJSON, createUSSDWrapper, rejectUSSDWrapper, TangoResponseUSSDWrapper } = require("./helper/helpers");
...
app.get("/tango-stream?:response", async (req, res) => {
  try {
        ...
        //SEND DATA TO TANGO
         ...
         console.log("Tango Response ")
          ...
          const {COMMAND: {MESSAGE}} = jsonObj;
           ...
           if (message === "Subscriber with this MSISDN is already registered in the system"){
            const USSDResponseFromTango = message;
            let responseConfirmWrapper = TangoResponseUSSDWrapper(USSDResponseFromTango);

            res.header({ "Content-Type": "text/xml", Accept: "text/xml" });
            res.send(responseConfirmWrapper);
            console.log(message);
            //sms call
            const { phone } = iPacsResponseData;
            smpp.SendSMSAlert2(phone,message);
           }else{
            const USSDSecondResponseFromTango = `${message} To complete your Orange Money registration, Send a copy of your valid ID card on whatsapp to 073235786 or 074013333`;
            let responseConfirmWrapper = TangoResponseUSSDWrapper(USSDSecondResponseFromTango);
    ...
    if (response === "Reject") {
      ...
      const userRejectContent = '<form action=/tango-stream?:response?:namevalue" method="GET"><entry type="text" var="namevalue"><prompt>Please Enter your First and Middle Name </prompt></entry></form>';
...
;  
      ...
      app.get("/tango-stream?:response?:namevalue", (req, res) => {
        const { namevalue } = req.query;
        
        if (typeof namevalue === "undefined" || !name) {
          return res.sendStatus(401);
        } 
        if (name === "Salim"){
            console.log(name)
        ...
        const rejectcontent2 ='<form action="/tangostream4" method="GET"><entry type="text" var="lastnamevalue"><prompt>Please Enter your Last Name </prompt></entry></form>';
