...
app.get("/tango-stream?:response", async (req, res) => {
  try {
    ...
    if (response === "Reject") {
      ...
      const userRejectContent = '<form action="/tango-stream3?:name" method="GET"><entry type="text" var="namevalue"><prompt>Please Enter your First and Middle Name </prompt></entry></form>';
...
;  
      ...
      //console.log(name)
      app.get("/tango-stream3?:name", (req, res) => {
        const { name } = req.query;

    if (typeof name === "undefined" || !response) {
      return res.sendStatus(401);
    }
